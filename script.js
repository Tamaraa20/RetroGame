    ...
    if (this.game.spriteUpdate) {
      this.game.waves.forEach((wave) => {
        wave.enemies.forEach((enemy) => {
          if (this.game.checkCollins(enemy, this)) {
            enemy.hit(this.damage);
          }
        });
      });
      this.game.bossArray.forEach((boss) => {
        if (this.game.checkCollins(boss, this) && boss.y >= 0) {
...
class Enemy {
  ...
  update(x, y) {
    ...
    this.game.projectilesPool.forEach((projectile) => {
      if (
        !projectile.free &&
        this.game.checkCollins(this, projectile) &&
        this.lives > 0
      ) {
        this.hit(1);
        projectile.reset();
    ...
    if (this.game.checkCollins(this, this.game.player) && this.lives > 0) {
      this.lives = 0;
...
class Boss {
  ...
  update() {
    ...
    // collision detected boss/player
    if (this.game.checkCollins(this, this.game.player) && this.lives >= 1) {
      this.game.gameOver = true;
      this.lives = 0;
    }
    // boss destroyed
...
class Game {
  ...
  render(context, deltaTime) {
    ...
    this.waves.forEach((wave) => {
      wave.render(context);
      if (wave.enemies.length < 1 && !wave.nexWaveTrigger && !this.gameOver) {
        this.newWave();
        wave.nexWaveTrigger = true;
      }
    });
  }
  createProjectiles() {
  ...
  drawStatusText(context) {
    ...
    for (let i = 0; i < this.player.energy; i++) {
      context.fillRect(20 + 2 * i, 130, 2, 20);
    }
    context.restore();
    if (this.gameOver) {
      context.textAlign = "center";
